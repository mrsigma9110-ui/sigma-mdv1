...
// IMPORTANT: every deployment gets its own MongoDB namespace. The previous
// version used the same Session/ActiveNumber collections for every clone, so
// one bot could reconnect another bot's WhatsApp sessions.
const instanceSeed = String(config.INSTANCE_ID || 'SIGMA_MD_DEFAULT');
const instanceTag = crypto.createHash('sha256').update(instanceSeed).digest('hex').slice(0, 16);
const collectionPrefix = `sigma_${instanceTag}`;
console.log(`🔒 SIGMA-MD isolated database namespace: ${instanceTag}`);
...
const sessionSchema = new mongoose.Schema({
    ...
    // Full Baileys multi-file auth state. This is required because creds.json
    // alone is not enough; the signal/pre-key files must survive container restarts.
    authFiles: {
        type: Object,
        default: null
    },
...
// Use per-instance collections instead of shared global collections.
// This is deliberately stronger than just adding an instance field because
// old unique indexes on the shared collections cannot cause cross-bot leaks.
const Session = mongoose.model(`Session_${instanceTag}`, sessionSchema, `${collectionPrefix}_sessions`);
const UserConfig = mongoose.model(`UserConfig_${instanceTag}`, userConfigSchema, `${collectionPrefix}_userconfig`);
const OTP = mongoose.model(`OTP_${instanceTag}`, otpSchema, `${collectionPrefix}_otp`);
const ActiveNumber = mongoose.model(`ActiveNumber_${instanceTag}`, activeNumberSchema, `${collectionPrefix}_active_numbers`);
const Stats = mongoose.model(`Stats_${instanceTag}`, statsSchema, `${collectionPrefix}_stats`);
...
// Sauvegarde session
async function saveSessionToMongoDB(number, credentials, authFiles = null) {
    try {
        ...
        if (authFiles && typeof authFiles === 'object') {
            update.authFiles = authFiles;
        }
        ...
        console.log(`📁 Session saved to MongoDB for ${cleanNumber}${authFiles ? ' (full auth state)' : ''}`);
        return true;
    } catch (error) {
...
// Return the complete auth state when available. Older sessions that only
// contain creds remain compatible and will be re-paired/re-saved normally.
async function getAuthStateFromMongoDB(number) {
    try {
        const cleanNumber = number.replace(/[^0-9]/g, '');
        const session = await Session.findOne({ number: cleanNumber });
        ...
        return {
            ...
            authFiles: session.authFiles || null
        };
    } catch (error) {
        console.error('❌ Error getting full auth state from MongoDB:', error);
        return null;
    }
}
